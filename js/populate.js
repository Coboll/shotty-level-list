...
for (var i = 0; i < list.length; i++) {
    for (var a = 0; a < list[i].vids.length; a++) {
        ...
        if (!list[i].vids[a].user == '') {
            ...
            rank_data.push({ link: list[i].vids[a].link, level: list[i].name, rank: i, name: list[i].vids[a].user, point: list[i].vids[a].percent == 100 ? getpoint(i + 1) : (getpoint(i + 1) - 2(i)) ** (list[i].vids[a].percent / 100 ) , percent: list[i].vids[a].percent })
        }
    }
